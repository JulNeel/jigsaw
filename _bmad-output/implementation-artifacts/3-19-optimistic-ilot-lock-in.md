...
- `pnpm build && pnpm lint && pnpm test` — clean after each task; 212 tests passing, no regressions. One expected transient warning (`addPredictedClusterLock` unused) between Task 2 and Task 3, resolved once Task 3 wired it in.

### Completion Notes List

- Task 1: `collections.ts`'s `onUpdate` now calls `emitMoveConflict(pieceId)` unconditionally on `!result.success` (removed the `isMove` guard and the now-dead `isMove` local + its own comment). `move-conflict-events.ts`'s header comment updated to describe the widened scope; exports/filename left unchanged per the story's own instruction.
- Task 2: New `PredictedClusterLock` type (module-level, alongside `PredictedFusion`). New `RoomCanvas` state `predictedClusterLocks`, `isClusterLockConfirmed` (cluster gone *or* `cluster.version >= sinceVersion`), `addPredictedClusterLock`/`activePredictedClusterLocks` (opportunistic pruning, mirroring `predictedFusions` exactly), and a `subscribeMoveConflict` effect matched on `representativePieceId`.
- Task 3: `ClusterGroupSprite` gained the `onPredictedClusterLock` prop. In `handleDragEnd`'s `if (slot)` branch, when `predictedLock` is true: computes `targetByPieceId` (same offset-difference formula already used for `predictFrameLock`'s own params), calls `onPredictedClusterLock` with the already-computed `expectedResultVersion`, and fires `onInstantFrameLockOutcome` (green pulse) for every member, not just the representative. Updated the two stale comments this story's own scope directly contradicted ("Unlike a solo piece, a Cluster never needs client-side prediction..." and "No colored pulse here (yet)..."). `RoomCanvas`'s `soloPieces`/`membersByClusterId` `useMemo` now diverts a genuinely-clustered piece into `solo` (with a patched `placedRow`/`placedCol` clone) whenever an active `PredictedClusterLock` covers it — reusing `pieceRenderPosition`'s existing `placedRow != null` priority and `SoloPieceSprite`'s existing `isPlaced` gating with zero new position/interaction code. Wired `onPredictedClusterLock={addPredictedClusterLock}` at the JSX call site.
- Task 4: `pnpm build && pnpm lint && pnpm test` all clean. Manual verification left unchecked — this environment has no browser to actually drag an Îlot onto a Frame slot and observe the instant snap/pulse, or to run the two-browser-session race case; needs the user's own pass per the story's Task 4 checklist.

### File List

- `src/lib/db/collections.ts` (modified — `emitMoveConflict` now unconditional on failure, dead `isMove` removed)
- `src/lib/rooms/move-conflict-events.ts` (modified — header comment only, no behavior change)
- `src/components/canvas/room-canvas.tsx` (modified — new `PredictedClusterLock` type, `predictedClusterLocks` state/logic in `RoomCanvas`, new `onPredictedClusterLock` prop on `ClusterGroupSprite`, per-member pulse, `soloPieces`/`membersByClusterId` derivation update)
...
| 2026-09-11 | Implemented all 3 tasks: widened `emitMoveConflict`, new `predictedClusterLocks` local-only override in `RoomCanvas`, wired into `ClusterGroupSprite.handleDragEnd` and the `soloPieces`/`membersByClusterId` render split. `pnpm build && pnpm lint && pnpm test` clean. Status → review; manual verification left to the user (no browser in this environment). |
