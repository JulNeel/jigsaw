...
> **Superseded 2026-09-21** — the scope reduction described in the next paragraph no longer applies; see this story's Change Log. A loose piece joining an existing Îlot is now grouped immediately too, and both halves are guarded by `e2e/optimistic-fusion.e2e.ts`. The paragraph is kept as written because it is what the original implementation actually did.
...
| Date | Change |
|------|--------|
| 2026-09-21 | **Task 2's scope reduction lifted.** The optimistic grouping no longer requires the stationary piece to be loose: `SoloPieceSprite`'s fused branch now uses `predictDropOutcome`'s `mergedMemberIds` — the fully expanded merged membership, whole Clusters included — which is what the original narrow version had no way to obtain. User report that prompted it: "lorsque j'associe plusieurs pièces libres, il y a immédiatement le son et le pulse, mais leur association visuelle prend parfois 2 ou 3 secondes" — that wait was the server's own confirmation arriving over Realtime, because in the Îlot case nothing was predicted at all. Guarded by `e2e/optimistic-fusion.e2e.ts` (verified red against the previous behaviour). AC #1 is now met without the caveat recorded below. |
