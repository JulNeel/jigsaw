...
- `src/components/canvas/room-canvas.tsx` (modified — `handleTouchStart`'s new single-finger-on-empty-space branch; `handleTouchMove`'s pinch/pan anchor math corrected; new `lastPinchMidpointRef`)
