...
| 2026-09-04 | Post-hoc bug fixes (user report on Firefox for Android, discovered while manually verifying an unrelated story): (1) pinch-to-zoom didn't register at all — a second finger landing could arrive after Konva's own native Stage drag had already engaged from the first finger alone, since toggling `draggable` via React state lags the synchronous touch event stream; fixed with an explicit, immediate `stage.stopDrag()` the instant a second touch is detected. (2) Once registering, the pinch stuttered badly on Firefox (smooth on Samsung Internet) — `handleTouchMove` called `setScale`/`setPosition` every single frame, forcing a full re-render at whatever rate Firefox fires uncoalesced `touchmove` events; fixed by moving the Konva Stage directly/imperatively during the gesture and syncing React state only once at gesture end, mirroring `handleDragEnd`'s existing pattern for panning. Both confirmed fixed by the user on-device. |
