...
- **Post-review fix (2026-09-05, user manual test):** pan direction was inverted — dragging toward an edge scrolled the view the *wrong* way. Root cause: `velocity` (from `computeAutoscrollVelocity`) is defined as "the direction the *view* should reveal more content" (e.g. positive x = pointer near the right edge = show more content to the right), but the Stage's own `position` is where content-space `(0,0)` lands on screen — revealing more content to the right requires moving that anchor *left*. The tick was applying `currentStagePos + velocity * dt` (same sign), which moves the Stage *with* the pointer's edge direction instead of *against* it. Fixed by subtracting: `currentStagePos - velocity * dt`. `computeAutoscrollVelocity`'s own semantics/tests are unchanged and correct (they describe the intended pan direction from the user's perspective, not the raw Stage-coordinate sign) — the bug was purely in how the tick applied that velocity to `stage.position()`.
...
| 2026-09-05 | User manual test found pan direction inverted. Fixed: the autoscroll tick was applying `velocity` to `stage.position()` with the wrong sign (added instead of subtracted) — corrected, `pnpm build && pnpm lint && pnpm test` re-verified clean. |
